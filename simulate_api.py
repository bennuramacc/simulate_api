...
# ─── 7) Baseline avg trip time ─────────────────────────────────────
def avg_trip(sc: Scenario, start: str, end: str) -> float:
    times = pd.date_range(start=start, end=end, freq="30min").time
    vals = [ one_trip(dt.datetime.combine(dt.date.today(),t), sc)["trip_time"]
             for t in times ]
    return float(np.mean(vals))

# ─── 8) Dynamic-day simulation ─────────────────────────────────────
def run_dynamic(sc: Scenario, start: str, end: str) -> pd.DataFrame:
    ...
    while dep <= end_dt:
        ...
        # headway logic vs. baseline
        diff = out["trip_time"] - base
        if   diff > 30:
            headway = 10; sc.bus_type = ARTIC
        elif diff > 20:
            headway = 10
        elif diff > 10:
            headway = 15
        ...
        # triple counts as requested
        out["max_occ"]  *= 3
        out["boarded"]  *= 3
        out["headway"]  = headway
        out["load_%"]   = round(100 * out["max_occ"] / out["capacity"], 2)

        # if still >90%, upgrade capacity & recompute load
        if out["load_%"] > 90:
