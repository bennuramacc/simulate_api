...
def get_lambda(stop: str, now: dt.datetime) -> float:
    ...
    for iv,val in row.items():
        s,e = iv.split('-');
        smin = int(s[:2])*60 + int(s[3:]); emin = int(e[:2])*60 + int(e[3:])
...
# ─── 5) CatBoost Modeli & Özellik Listesi ─────────────────────────────
...
FEATS = [
 ...
 'weather_description_Cloudy','weather_description_Low Visibility','weather_description_Precipitation','weather_description_Storm',
...
def make_feats(now: dt.datetime, sc: 'Scenario', km: float, lags: List[float]) -> List[float]:
    ...
    wd_key = f'f"weather_description_{sc.weather_desc}"'
    if wd_key in f: f[wd_key] = 1
    l = (lags + [0]*5)[:5]
    for i,v in enumerate(l,1): f[f'HATSURESI_LAG_{i}'] = v
    return [f[n] for n in FEATS]
...
def one_trip(dep: dt.datetime, sc: Scenario) -> dict:
    ...
    class Bus:
        def __init__(self, env, dep, sc):
            ...
            self.pax=[]; self.max_occ=0; self.boarded=0; self.lags=[0]*5; env.process(self.run())
        def run(self):
            yield self.env.timeout((self.dep-dt.datetime.combine(self.dep.date(),dt.time())).total_seconds()/60)
            total=0.0
            for r in self.seg.itertuples():
                # travel
                now,km=self.curr,r.km
                feat=[make_feats(now,self.sc,km,self.lags)]
                sec=float(CAT.predict(pd.DataFrame(feat,columns=FEATS))[0]); dur=sec/60
                self.lags=[dur]+self.lags[:4]; total+=dur
                yield self.env.timeout(dur); self.curr+=dt.timedelta(minutes=dur)
                # in-out
                out=[p for p in self.pax if p==r.stop]; self.pax=[p for p in self.pax if p!=r.stop]
                lam=get_lambda(r.stop,now)*ARR_SCALE*self.sc.demand_multiplier
                if self.sc.is_public_holiday: lam*=0.8
                nin=np.random.poisson(lam*dur)
                dests,probs=DEST.get(r.stop,([],[]))
                new=(np.random.choice(dests,size=nin,p=probs).tolist() if dests else [r.stop]*nin)
                self.pax.extend(new); self.boarded+=nin; self.max_occ=max(self.max_occ,len(self.pax))
                # dwell
                dwell=r.dwell_sec/60+len(out)*0.03+nin*0.01
                if self.sc.is_school_day: dwell*=1.1
                if self.sc.is_public_holiday: dwell*=1.2
                total+=dwell; yield self.env.timeout(dwell); self.curr+=dt.timedelta(minutes=dwell)
            LOG.append({'depart_time':self.dep.strftime('%H:%M'),'bus_type':self.sc.bus_type.name,
                        'capacity':self.sc.bus_type.capacity,'trip_time':round(total,2),
                        'max_occ':self.max_occ,'boarded':self.boarded})
    Bus(env,dep,sc); env.run(); return LOG[0]

# ─── 8) Ortalama Trip Süresi ────────────────────────────────────────
def avg_trip(sc: Scenario, start: str, end: str) -> float:
    times=pd.date_range(start=start,end=end,freq='30min').time
    return np.mean([one_trip(dt.datetime.combine(dt.date.today(),t),sc)['trip_time'] for t in times])

# ─── 9) Dinamik Slot-Day ──────────────────────────────────────────
CURRENT_THR=90
def run_dynamic(sc: Scenario, start: str='06:00', end: str='23:00') -> pd.DataFrame:
    base=avg_trip(sc,start,end); recs=[]; headway=30
    ...
    while dep<=end_dt:
        ...
        out=one_trip(dep,sc); out['headway']=headway
        out['max_occ']*=6; out['boarded']*=5
        out['load_%']=round(100*out['max_occ']/out['capacity'],2)
        if out['load_%']>90:
            sc.bus_type=ARTIC; out['bus_type']=ARTIC.name; out['capacity']=ARTIC.capacity
            out['load_%']=round(100*out['max_occ']/out['capacity'],2)
        recs.append(out)
        diff=out['trip_time']-base
        headway=7 if diff>2.5 else (10 if diff>1 else (15 if diff>0.5 else 20))
...
def simulate(req: SimRequest):
    sc=Scenario(weather_desc=req.weather_desc, temp=req.temp, demand_multiplier=req.demand_multiplier,
                is_school_day=req.is_school_day, is_public_holiday=req.is_public_holiday,
                is_pandemic=req.is_pandemic, bus_type=STD)
